...
public class PackageBenefitServiceImpl implements PackageBenefitService {
    private static final Logger log = LoggerFactory.getLogger(PackageBenefitServiceImpl.class);
    @Resource
    ...
    @Override
    public Boolean create(PackageBenefitPO po) {
        log.info("create package_benefit id={}", po.getId());
        return mapper.insert(po) > 0;
    }

    @Override
    public Boolean update(PackageBenefitPO po) {
        log.info("update package_benefit id={}", po.getId());
        return mapper.updateById(po) > 0;
    }

    @Override
    public Boolean delete(Long id) {
        log.warn("delete package_benefit id={}", id);
