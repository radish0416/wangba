/**
 * 本文件由工具自动生成，请勿手工修改。
 * 生成器: tools/codegen/generate_from_sql.py
 */
package com.cybercafe.service.impl;
import com.cybercafe.service.OrderMasterService;
import com.cybercafe.repository.mapper.OrderMasterMapper;
import com.cybercafe.domain.po.OrderMasterPO;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.cybercafe.web.dto.OrderMasterQueryDTO;
import org.springframework.stereotype.Service;
import javax.annotation.Resource;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/** 订单主表 服务实现。 */
@Service
public class OrderMasterServiceImpl implements OrderMasterService {
    private static final Logger log = LoggerFactory.getLogger(OrderMasterServiceImpl.class);
    @Resource
    private OrderMasterMapper mapper;

    @Override
    public IPage<OrderMasterPO> listPage(OrderMasterQueryDTO dto) {
        Page<?> page = new Page<>(dto.current == null ? 1 : dto.current, dto.size == null ? 10 : dto.size);
        return mapper.selectByDynamic(page, dto);
    }

    @Override
    public java.util.List<OrderMasterPO> listAll(OrderMasterQueryDTO dto) {
        return mapper.selectByDynamic(new Page<>(1, Integer.MAX_VALUE), dto).getRecords();
    }

    @Override
    public OrderMasterPO detail(Long id) {
        return mapper.selectDetail(id);
    }

    @Override
    public Boolean create(OrderMasterPO po) {
        log.info("create order_master id={}", po.getId());
        return mapper.insert(po) > 0;
    }

    @Override
    public Boolean update(OrderMasterPO po) {
        log.info("update order_master id={}", po.getId());
        return mapper.updateById(po) > 0;
    }

    @Override
    public Boolean delete(Long id) {
        log.warn("delete order_master id={}", id);
        return mapper.deleteById(id) > 0;
    }
}
