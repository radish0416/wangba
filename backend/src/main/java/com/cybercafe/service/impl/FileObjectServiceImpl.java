/**
 * 本文件由工具自动生成，请勿手工修改。
 * 生成器: tools/codegen/generate_from_sql.py
 */
package com.cybercafe.service.impl;
import com.cybercafe.service.FileObjectService;
import com.cybercafe.repository.mapper.FileObjectMapper;
import com.cybercafe.domain.po.FileObjectPO;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.cybercafe.web.dto.FileObjectQueryDTO;
import org.springframework.stereotype.Service;
import javax.annotation.Resource;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/** 文件对象 服务实现。 */
@Service
public class FileObjectServiceImpl implements FileObjectService {
    private static final Logger log = LoggerFactory.getLogger(FileObjectServiceImpl.class);
    @Resource
    private FileObjectMapper mapper;

    @Override
    public IPage<FileObjectPO> listPage(FileObjectQueryDTO dto) {
        Page<?> page = new Page<>(dto.current == null ? 1 : dto.current, dto.size == null ? 10 : dto.size);
        return mapper.selectByDynamic(page, dto);
    }

    @Override
    public java.util.List<FileObjectPO> listAll(FileObjectQueryDTO dto) {
        return mapper.selectByDynamic(new Page<>(1, Integer.MAX_VALUE), dto).getRecords();
    }

    @Override
    public FileObjectPO detail(Long id) {
        return mapper.selectDetail(id);
    }

    @Override
    public Boolean create(FileObjectPO po) {
        log.info("create file_object id={}", po.getId());
        return mapper.insert(po) > 0;
    }

    @Override
    public Boolean update(FileObjectPO po) {
        log.info("update file_object id={}", po.getId());
        return mapper.updateById(po) > 0;
    }

    @Override
    public Boolean delete(Long id) {
        log.warn("delete file_object id={}", id);
        return mapper.deleteById(id) > 0;
    }
}
