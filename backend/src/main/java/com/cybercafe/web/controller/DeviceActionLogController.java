...
/** 设备动作日志 控制器。 */
@RestController
@RequestMapping("/api/device_action_log")
@Tag(name = "设备动作日志")
public class DeviceActionLogController {
    @Resource
    private DeviceActionLogService service;

@PostMapping("/listPage")
@Operation(summary = "分页查询")
    public ApiResponse<IPage<DeviceActionLogPO>> listPage(@RequestBody @Validated DeviceActionLogQueryDTO dto) {
        return ApiResponse.ok(service.listPage(dto));
    }

@PostMapping("/listAll")
@Operation(summary = "全量查询")
    public ApiResponse<java.util.List<DeviceActionLogPO>> listAll(@RequestBody @Validated DeviceActionLogQueryDTO dto) {
        return ApiResponse.ok(service.listAll(dto));
    }

@PostMapping("/detail")
@Operation(summary = "详情")
    public ApiResponse<DeviceActionLogPO> detail(@RequestBody java.util.Map<String, Long> req) {
        return ApiResponse.ok(service.detail(req.get("id")));
    }

@PostMapping("/create")
@Operation(summary = "新增")
    public ApiResponse<Boolean> create(@RequestBody @Validated DeviceActionLogPO po) {
        return ApiResponse.ok(service.create(po));
    }

@PostMapping("/update")
@Operation(summary = "修改")
    public ApiResponse<Boolean> update(@RequestBody @Validated DeviceActionLogPO po) {
