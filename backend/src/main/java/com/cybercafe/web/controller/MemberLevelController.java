...
/** 会员等级 控制器。 */
@RestController
@RequestMapping("/api/member_level")
@Tag(name = "会员等级")
...
@Operation(summary = "分页查询")
    public ApiResponse<IPage<MemberLevelPO>> listPage(@RequestBody @Validated MemberLevelQueryDTO dto) {
        return ApiResponse.ok(service.listPage(dto));
    }

@PostMapping("/listAll")
@Operation(summary = "全量查询")
    public ApiResponse<java.util.List<MemberLevelPO>> listAll(@RequestBody @Validated MemberLevelQueryDTO dto) {
        return ApiResponse.ok(service.listAll(dto));
    }

@PostMapping("/detail")
@Operation(summary = "详情")
    public ApiResponse<MemberLevelPO> detail(@RequestBody java.util.Map<String, Long> req) {
        return ApiResponse.ok(service.detail(req.get("id")));
    }

@PostMapping("/create")
@Operation(summary = "新增")
    public ApiResponse<Boolean> create(@RequestBody @Validated MemberLevelPO po) {
        return ApiResponse.ok(service.create(po));
    }

@PostMapping("/update")
@Operation(summary = "修改")
    public ApiResponse<Boolean> update(@RequestBody @Validated MemberLevelPO po) {
